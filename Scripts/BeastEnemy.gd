...
func _on_AnimationPlayer_animation_finished(anim_name):
	...
	if anim_name == "death":
		var drop = load("res://Scenes/DropItems.tscn").instance()
		drop.drop_type = 1
		drop.global_position = self.position
...
func _on_Hurtbox_area_entered(_area):
	if health == max_health*0.7 or health == max_health*0.2:
		is_hit = true
	health -= 1
	if health < 1:
...
func _on_Hitbox_body_entered(body):
	if body.name == "Player":
		...
		body.velocity.y = -knockback_dir * knockback_power
